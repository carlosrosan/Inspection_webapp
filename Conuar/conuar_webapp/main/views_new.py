...
def inspection_detail(request, inspection_id):
    ...
    arrow_details = list(inspection.arrow_details.order_by('row_index'))
...
def _generate_arrow_chart_base64(arrow_details, axis='xc', title='Vista frontal - Xc'):
    ...
    try:
        ...
        rows = [r.row_index for r in arrow_details]
        values = [getattr(r, axis) for r in arrow_details if getattr(r, axis) is not None]
        valid_rows = [r.row_index for r in arrow_details if getattr(r, axis) is not None]
...
def generate_inspection_pdf_to_file(inspection_id, save_to_disk=True):
    ...
    # Arrow details
    from .models import ArrowDetail
    arrow_details = list(ArrowDetail.objects.filter(inspection=inspection).order_by('row_index'))
